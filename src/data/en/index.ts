export const tEn = {
  "header": {
    "projects": "Projects",
    "contacts": "Contacts",
    "main_page": "main",
    "portfolio_page": "projects",
    "contact_page": "contacts"
  },
  "hero": {
    "title": "Creating Websites for Business",
    "view": "View",
    "cases": "All Cases",
    "stat_1": {
      "num": '25',
      "text": 'professionals in the team'
    },
    "stat_2": {
      "num": '34',
      "text": 'successful projects'
    },
    "stat_3": {
      "num": '126+',
      "text": 'years of combined experience'
    }
  },
  "services": {
    "title": "Services",
    "text": {
      "t_1": "Thanks to a large team, we can create any IT product from 0 to an ideal final",
      "t_2": "Ability to track project stages in real-time on Trello",
      "t_3": "Check out our experience",
      "button": "Projects"
    },
    "items": [
      {
        "title": "Websites",
        "list": ["Landing page", "Multipage websites", "WEB applications", "Online stores"],
        "isOpen": false
      },
      {
        "title": "Design",
        "list": ["Web design", "Mobile design", "Logos", "Brand book", "Corporate identity"],
        "isOpen": false
      },
      {
        "title": "Mobile Applications",
        "list": ["Mobile games", "Content services", "Promo applications"],
        "isOpen": false
      },
      {
        "title": "Other",
        "list": ["Chatbots", "WEB3 products", "Parsers", "Automation"],
        "isOpen": false
      }
    ]
  },
  contacts: {
    "title": "Contact us",
    "title_2": "Any questions remaining?",
    "text": "Let's discuss your idea",
    "label": {
      "service": "Service of interest",
      "name": "Name",
      "mail": "Email",
      "phone": "Phone",
      "wishes": "Comment",
      "files": "Uploading additional files",
      "submit": "Send",
      "error": "Required fields not filled in. (Marked with an asterisk)"
    }
  },
  stack: {
    "title": "Stack",
    "items": [
      {
        title: "WEB Frontend",
        items: ["HTML5 + CSS3", " LESS / Sass / Scss", "Bootstrap / Tailwind", "JavaScript / TypeScript", "jQuery", "React / Vue / Next / Nuxt", "Gulp / Webpack / Grun"]
      },
      {
        title: "Backend Development",
        items: ["Node.JS / Nest.JS", "TypeScript", "PHP", "Python", "Java", "Golang", "Docker", "Laravel / Yii2", "PostgreSQL / MySQL / MongoDB / GraphQL / Appwrite", "Swagger / Postman", "CI / CD", "Solidity / Smart Contracts / Blockchain"]
      },
      {
        title: "Mobile Development",
        items: ["FlutterFlow", "ReactNative", "C++", "C#", "Java"]
      },
      {
        title: "CMS",
        items: ["Bitrix24", "WordPress", "Shopify", "Tilda", "OpenCart"]
      },
      {
        title: "Design",
        items: ["Figma", "Photoshop", "Illustrator", "Premiere Pro", "InDesign"]
      },
      {
        title: "Teamwork",
        items: ["Linux / Windows / macOS", "Git", "GitHub", "Vercel", "Atlassian Jira / Basecamp / Toggle Track / Trello / Notion"]
      }
    ]
  },
  projects: {
    "title": "Projects",
    "best": "Best projects",
    "button": "Go to website",
    "stat_1": "hours",
    "stat_2": "developers",
    "stat_3": "designers",
    "stack_title": "Select the stack",
    "service_title": "Select the service",
    "service_filter": "Service",
    "stack_filter": "Stack",
    "reset_filter": "Reset",
    "apply_filter": "Apply",
    "hide_filter": "Hide",
    "not_found":
      `We don't have such a project yet.\n(But we will definitely improve!)😉`,
    "items": [
      {
        "id": "01",
        "name": "KSD Company",
        "task": "Creating design for a landing page.",
        "description": "",
        "service": [
          "Web design"
        ],
        "stack": [
          "Figma",
          "Photoshop"
        ],
        "path": "ksd",
        "url": "https://www.figma.com/file/q1v1ct7QfqR5JuIbZ5Hzd8/Said?type=design&node-id=701%3A9&mode=design&t=EhMeUG0PV2jhpVtL-1",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": true,
        "statistic": {
          "hours": 18,
          "dev": 0,
          "des": 2
        }
      },
      {
        "id": "02",
        "name": "Wonder Needle",
        "task": "Design and layout of an online store.",
        "description": "",
        "service": [
          "Web design"
        ],
        "stack": [
          "HTML",
          "CSS",
          "Bootstrap",
          "JavaScript",
          "JQuery",
          "SCSS",
          "Illustrator"
        ],
        "path": "chudo-igla",
        "url": "https://chudo-igla.ru/",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 36,
          "dev": 1,
          "des": 2
        }
      },
      {
        "id": "03",
        "name": "Invoice Profi",
        "task": "Layout and integration into WordPress.",
        "description": "Our team successfully adapted the existing design, enhancing it according to our requirements and expectations. Additionally, we created an innovative audio player from scratch, entirely based on JavaScript technology. Moreover, we developed a plugin for the site providing functionality for pricing, access management, and tracking of uploaded files. An important step was also the creation of a mechanism for sending invoices and reports in PDF format for our client's system.",
        "service": [
          "Multipage websites"
        ],
        "stack": [
          "HTML",
          "CSS",
          "WordPress",
          "JavaScript",
          "Node.JS"
        ],
        "path": "invoice-profi",
        "url": "https://invoiceprofi.ru/",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": true,
        "statistic": {
          "hours": 25,
          "dev": 2,
          "des": 1
        }
      },
      {
        "id": "04",
        "name": "Sens Collective",
        "task": "Development of a new page on CMS 1C-Bitrix",
        "description": "Our team successfully adapted the existing design, enhancing it according to our requirements and expectations. Additionally, we created an innovative audio player from scratch, entirely based on JavaScript technology. Moreover, we developed a plugin for the site providing functionality for pricing, access management, and tracking of uploaded files. An important step was also the creation of a mechanism for sending invoices and reports in PDF format for our client's system.",
        "service": [
          "Multipage websites"
        ],
        "stack": [
          "HTML",
          "CSS",
          "JavaScript",
          "PHP",
          "1C-Bitrix",
          "Figma"
        ],
        "path": "sens-collective",
        "url": "sens-collective.com/",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": true,
        "statistic": {
          "hours": 10,
          "dev": 1,
          "des": 1
        }
      },
      {
        "id": "05",
        "name": "Ivolga",
        "task": "Development of an online store from scratch.",
        "description": "",
        "service": [
          "Online stores"
        ],
        "stack": [
          "HTML",
          "SCSS",
          "JavaScript",
          "PHP",
          "1C-Bitrix",
          "Figma"
        ],
        "path": "ivolga",
        "url": "",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": true,
        "statistic": {
          "hours": 45,
          "dev": 4,
          "des": 2
        }
      },
      {
        "id": "06",
        "name": "Two Biscuits",
        "task": "Design and development of an online store.",
        "description": "",
        "service": [
          "Online stores",
          "Web design"
        ],
        "stack": [
          "HTML",
          "SCSS",
          "Gulp",
          "JavaScript",
          "JQuery",
          "Illustrator",
          "Photoshop"
        ],
        "path": "dva-biskvita",
        "url": "https://dvabiskvita.ru/",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 28,
          "dev": 2,
          "des": 2
        }
      },
      {
        "id": "07",
        "name": "Goods Mood",
        "task": "Logo for a store.",
        "description": "",
        "service": [
          "Logos"
        ],
        "stack": [
          "Illustrator"
        ],
        "path": "goods-mood",
        "url": "",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 6,
          "dev": 0,
          "des": 1
        }
      },
      {
        "id": "08",
        "name": "Sergey Krupnov Studio",
        "task": "Logo for a jewelry studio.",
        "description": "",
        "service": [
          "Logos"
        ],
        "stack": [
          "Illustrator"
        ],
        "path": "krupnov-studio",
        "url": "",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 4,
          "dev": 0,
          "des": 1
        }
      },
      {
        "id": "09",
        "name": "Neo Cryo",
        "task": "Logo for a startup.",
        "description": "",
        "service": [
          "Logos"
        ],
        "stack": [
          "Illustrator"
        ],
        "path": "neo-cryo",
        "url": "",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 5,
          "dev": 0,
          "des": 1
        }
      },
      {
        "id": "10",
        "name": "Rivulet Digital",
        "task": "Logo for a startup.",
        "description": "",
        "service": [
          "Logos"
        ],
        "stack": [
          "Illustrator"
        ],
        "path": "rivulet-digital",
        "url": "",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 12,
          "dev": 0,
          "des": 1
        }
      },
      {
        "id": "11",
        "name": "Backeland",
        "task": "Banners for social media.",
        "description": "",
        "service": [
          "Web design"
        ],
        "stack": [
          "Illustrator",
          "Photoshop"
        ],
        "path": "backeland",
        "url": "",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 3,
          "dev": 0,
          "des": 1
        }
      },
      {
        "id": "12",
        "name": "Backpack Autotrade",
        "task": "Bot for increase trading volume on crypto exchanges",
        "description": "",
        "service": [
          "Parsers"
        ],
        "stack": [
          "Python"
        ],
        "path": "backpack-autotrade",
        "url": "",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 32,
          "dev": 1,
          "des": 0
        }
      }
    ]
  }
}