export const tFr = {
  "header": {
    "projects": "Projets",
    "contacts": "Contacts",
    "main_page": "accueil",
    "portfolio_page": "projets",
    "contact_page": "contacts"
  },
  "hero": {
    "title": "Création de sites Web pour les entreprises",
    "view": "Voir",
    "cases": "Tous les cas",
    "stat_1": {
      "num": '25',
      "text": 'professionnels dans l\'équipe'
    },
    "stat_2": {
      "num": '34',
      "text": 'projets réussis'
    },
    "stat_3": {
      "num": '126+',
      "text": 'années d\'expérience combinée'
    }
  },
  "services": {
    "title": "Services",
    "text": {
      "t_1": "Grâce à une grande équipe, nous pouvons créer n'importe quel produit informatique de 0 à un résultat final idéal",
      "t_2": "Possibilité de suivre les étapes du projet en temps réel sur Trello",
      "t_3": "Découvrez notre expérience",
      "button": "Projets"
    },
    "items": [
      {
        "title": "Sites Web",
        "list": ["Page de destination", "Sites Multipages", "Applications WEB", "Boutiques en ligne"],
        "isOpen": false
      },
      {
        "title": "Design",
        "list": ["Design web", "Design mobile", "Logos", "Cahier de marque", "Identité d'entreprise"],
        "isOpen": false
      },
      {
        "title": "Applications Mobiles",
        "list": ["Jeux mobiles", "Services de contenu", "Applications promotionnelles"],
        "isOpen": false
      },
      {
        "title": "Autres",
        "list": ["Chatbots", "Produits WEB3", "Parsers", "Automatisation"],
        "isOpen": false
      }
    ]
  },
  contacts: {
    "title": "Nous contacter",
    "title_2": "Des questions restent-elles",
    "text": "Discutons de votre idée",
    "label": {
      "service": "Service d'intérêt",
      "name": "Nom",
      "mail": "Email",
      "phone": "Téléphone",
      "wishes": "Commentaire",
      "files": "Téléchargement de fichiers supplémentaires",
      "submit": "Envoyer",
      "error": "Champs obligatoires non remplis. (Marqués d'un astérisque)"
    }
  },
  stack: {
    "title": "Empilement",
    "items": [
      {
        "title": "Frontend Web",
        "items": ["HTML5 + CSS3", "LESS / Sass / Scss", "Bootstrap / Tailwind", "JavaScript / TypeScript", "jQuery", "React / Vue / Next / Nuxt", "Gulp / Webpack / Grunt"]
      },
      {
        "title": "Développement Backend",
        "items": ["Node.JS / Nest.JS", "TypeScript", "PHP", "Python", "Java", "Golang", "Docker", "Laravel / Yii2", "PostgreSQL / MySQL / MongoDB / GraphQL / Appwrite", "Swagger / Postman", "CI / CD", "Solidity / Smart Contracts / Blockchain"]
      },
      {
        "title": "Développement Mobile",
        "items": ["FlutterFlow", "ReactNative", "C++", "C#", "Java"]
      },
      {
        "title": "CMS",
        "items": ["Bitrix24", "WordPress", "Shopify", "Tilda", "OpenCart"]
      },
      {
        "title": "Design",
        "items": ["Figma", "Photoshop", "Illustrator", "Premiere Pro", "InDesign"]
      },
      {
        "title": "Travail d'équipe",
        "items": ["Linux / Windows / macOS", "Git", "GitHub", "Vercel", "Atlassian Jira / Basecamp / Toggle Track / Trello / Notion"]
      }
    ]
  },
  projects: {
    "title": "Projets",
    "best": "Meilleurs projets",
    "button": "Aller sur le site",
    "stat_1": "heures",
    "stat_2": "développeurs",
    "stat_3": "concepteurs",
    "stack_title": "Sélectionner la pile",
    "service_title": "Sélectionner le service",
    "service_filter": "Service",
    "stack_filter": "Pile",
    "reset_filter": "Réinitialiser",
    "apply_filter": "Appliquer",
    "hide_filter": "Masquer",
    "not_found": 
    `Nous n'avons pas encore ce projet.\n(Mais nous nous améliorerons certainement !)😉`,
    "items": [
      {
        "id": "01",
        "name": "KSD Company",
        "task": "Création de design pour une page d'accueil.",
        "description": "",
        "service": [
          "Design Web"
        ],
        "stack": [
          "Figma",
          "Photoshop"
        ],
        "path": "ksd",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": true,
        "statistic": {
          "hours": 18,
          "dev": 0,
          "des": 2
        }
      },
      {
        "id": "02",
        "name": "Aiguille Miracle",
        "task": "Design et mise en page d'une boutique en ligne.",
        "description": "",
        "service": [
          "Design Web"
        ],
        "stack": [
          "HTML",
          "CSS",
          "Bootstrap",
          "JavaScript",
          "JQuery",
          "SCSS",
          "Illustrator"
        ],
        "path": "chudo-igla",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 36,
          "dev": 1,
          "des": 2
        }
      },
      {
        "id": "03",
        "name": "Invoice Profi",
        "task": "Mise en page et intégration WordPress.",
        "description": "Notre équipe a réussi à adapter un design existant en le perfectionnant selon nos besoins et attentes. De plus, nous avons créé un lecteur audio innovant à partir de zéro, entièrement basé sur la technologie JavaScript. De manière additionnelle, nous avons développé un plugin pour le site, fournissant des fonctionnalités de tarification, de gestion d'accès et de suivi des fichiers téléchargés. Un pas important a également été la création d'un mécanisme d'envoi de factures et de rapports au format PDF pour notre système client.",
        "service": [
          "Sites multipages"
        ],
        "stack": [
          "HTML",
          "CSS",
          "WordPress",
          "JavaScript",
          "Node.JS"
        ],
        "path": "invoice-profi",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": true,
        "statistic": {
          "hours": 25,
          "dev": 2,
          "des": 1
        }
      },
      {
        "id": "04",
        "name": "Sens collective",
        "task": "Développement d'une nouvelle page sur CMS 1C-Bitrix",
        "description": "Notre équipe a réussi à adapter un design existant en le perfectionnant selon nos besoins et attentes. De plus, nous avons créé un lecteur audio innovant à partir de zéro, entièrement basé sur la technologie JavaScript. De manière additionnelle, nous avons développé un plugin pour le site, fournissant des fonctionnalités de tarification, de gestion d'accès et de suivi des fichiers téléchargés. Un pas important a également été la création d'un mécanisme d'envoi de factures et de rapports au format PDF pour notre système client.",
        "service": [
          "Sites Multipages"
        ],
        "stack": [
          "HTML",
          "CSS",
          "JavaScript",
          "PHP",
          "1C-Bitrix",
          "Figma"
        ],
        "path": "sens-collective",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": true,
        "statistic": {
          "hours": 10,
          "dev": 1,
          "des": 1
        }
      },
      {
        "id": "05",
        "name": "Ivolga",
        "task": "Développement d'une boutique en ligne à partir de zéro.",
        "description": "",
        "service": [
          "Boutiques en ligne"
        ],
        "stack": [
          "HTML",
          "SCSS",
          "JavaScript",
          "PHP",
          "1C-Bitrix",
          "Figma"
        ],
        "path": "ivolga",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": true,
        "statistic": {
          "hours": 45,
          "dev": 4,
          "des": 2
        }
      },
      {
        "id": "06",
        "name": "Deux Biscuits",
        "task": "Design et développement d'une boutique en ligne.",
        "description": "",
        "service": [
          "Boutiques en ligne",
          "Design Web"
        ],
        "stack": [
          "HTML",
          "SCSS",
          "Gulp",
          "JavaScript",
          "JQuery",
          "Illustrator",
          "Photoshop"
        ],
        "path": "dva-biskvita",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 28,
          "dev": 2,
          "des": 2
        }
      },
      {
        "id": "07",
        "name": "Goods Mood",
        "task": "Logo pour une boutique.",
        "description": "",
        "service": [
          "Logos"
        ],
        "stack": [
          "Illustrator"
        ],
        "path": "goods-mood",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 6,
          "dev": 0,
          "des": 1
        }
      },
      {
        "id": "08",
        "name": "Studio de Sergey Krupnov",
        "task": "Logo pour un studio de joaillerie.",
        "description": "",
        "service": [
          "Logos"
        ],
        "stack": [
          "Illustrator"
        ],
        "path": "krupnov-studio",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 4,
          "dev": 0,
          "des": 1
        }
      },
      {
        "id": "09",
        "name": "Néo Cryo",
        "task": "Logo pour une startup.",
        "description": "",
        "service": [
          "Logos"
        ],
        "stack": [
          "Illustrator"
        ],
        "path": "neo-cryo",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 5,
          "dev": 0,
          "des": 1
        }
      },
      {
        "id": "10",
        "name": "Rivulet Digital",
        "task": "Logo pour une startup.",
        "description": "",
        "service": [
          "Logos"
        ],
        "stack": [
          "Illustrator"
        ],
        "path": "rivulet-digital",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 12,
          "dev": 0,
          "des": 1
        }
      },
      {
        "id": "11",
        "name": "Backeland",
        "task": "Bannières pour les réseaux sociaux.",
        "description": "",
        "service": [
          "Design Web"
        ],
        "stack": [
          "Illustrator",
          "Photoshop"
        ],
        "path": "backeland",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 3,
          "dev": 0,
          "des": 1
        }
      },
      {
        "id": "12",
        "name": "Backpack Autotrade",
        "task": "Bot pour augmenter le volume sur une bourse de cryptomonnaies.",
        "description": "",
        "service": [
          "Parsers"
        ],
        "stack": [
          "Python"
        ],
        "path": "backpack-autotrade",
        "images": [
          "preview.png",
          "1.png",
          "2.png"
        ],
        "best": false,
        "statistic": {
          "hours": 32,
          "dev": 1,
          "des": 0
        }
      }
    ]
  }
}